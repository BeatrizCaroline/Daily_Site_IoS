...
// Rastreia quais seções estão visíveis e qual a mais alta
const visibleSections = new Set();

function updateActive() {
  if (visibleSections.size === 0) return;

  // Dentre as visíveis, pega a que aparece primeiro no DOM
  let topSection = null;
  sections.forEach(sec => {
    if (visibleSections.has(sec.id)) {
      if (!topSection) topSection = sec.id;
    }
  });

  sidebarLinks.forEach(l => l.classList.remove('active'));
  if (topSection && linkMap[topSection]) {
    linkMap[topSection].classList.add('active');
  }
}

// Cria o observer apontado para o container scrollável (#privBody)
const observer = new IntersectionObserver(
  ...
  {
    root: bodyEl,          // observa dentro do container, não da viewport
    rootMargin: '0px 0px -60% 0px', // seção precisa estar no terço superior para ativar
    threshold: 0,
  }
);
...
sidebarLinks.forEach(link => {
  link.addEventListener('click', e => {
    ...
    // Força highlight imediato ao clicar
    sidebarLinks.forEach(l => l.classList.remove('active'));
    ...
    const offset       = sectionTop - containerTop + bodyEl.scrollTop - 20;
